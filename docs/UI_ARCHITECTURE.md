...
- **long_rest / short_rest** — cooldown אמיתי: אחרי מהלך/קפיצה הכלי נכנס למנוחה,
  **אסור לו לזוז** (`Reason.RESTING`) ואי אפשר לבחור בו. ה‑arbiter חושף את
  `cooldown_progress` (0→1), וה‑renderer מצייר מעליו **צעיף אדום שנגלל מלמעלה
  למטה**: הוא מכסה את התא במלואו כשהמנוחה מתחילה ומתרוקן כלפי מטה עד שנעלם ברגע
  שהמנוחה נגמרת — כך גובה הצעיף משמש כמד‑זמן שנותר. משכי המנוחה ב‑`config/settings.py`;
  אם מציבים 0 — אין cooldown (התנהגות המסלול הפקודתי/VPL נשמרת).
