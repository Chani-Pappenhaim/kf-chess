...
Loading lives here, apart from Img (the drawing primitive). Images are read
with ``np.fromfile`` + ``cv2.imdecode`` rather than ``cv2.imread``, which
cannot open paths containing non-ASCII characters on Windows.
"""
...
def read_image(path, size=None, keep_aspect=False, interpolation=cv2.INTER_AREA):
    ...
    exact resize to ``size`` by default, or shrink-to-fit preserving aspect
    ratio when ``keep_aspect`` is True."""
